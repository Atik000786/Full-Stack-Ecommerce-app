...
export const filterProductController= async(req,res)=>{
   try{
        ...
        res.status(200).send({
          success:true,
          message:"successfully product filtered",
          products 
        })
   }catch(error){
     console.log(error);
     res.status(500).send({
         success:false,
         message:"Error in Product Filter process"
     });
   }
};
 // count total product
...
export const searchProductController= async(req,res)=>{
  try{
     ...
     res.status(500).send({
      success:false,
      message:"Erron in searching",
      error
     });
  }
};
// related smillar products get
...
export const relatedProductController= async(req,res)=>{
   try{
     ...
     res.status(200).send({
      success:false,
      message:"Error in realed product controller",
      error
     })
   }
};
...
export const BraintreePaymentController=async (req,res)=>{
  try{
     ...
     let newTransiction= gateway.transaction.sale({
      amount:total,
